...
//        print("transactions from model - \(transactions)")
        var amount: Float = 0
        print("transactions.count - \(transactions.count)")
        for transaction in transactions {
            for (category, codes) in self.mmc {
                for code in codes {
                    if transaction.mcc ?? 0 == code {
                        if amountPerCategory[category] != nil {
//                            let amount = amountPerCategory[category]!
//                            let sum = amount + Float((transaction.amount! / 100))
//                            amountPerCategory.updateValue(sum, forKey: category)
                            amountPerCategory[category]! += Float((transaction.amount! / 100))
                        } else {
//                            let value = Float((transaction.amount! / 100))
//                            amountPerCategory.updateValue(value, forKey: category)
...
//                    else {
//                        print("CODE! - \(transaction.mcc)")
//                        print("transaction amount - \(transaction.amount)")
//                        if amountPerCategory[.other] != nil {
////                            let amount = amountPerCategory[.other]!
////                            let sum = amount + Float((transaction.amount! / 100))
////                            amountPerCategory.updateValue(sum, forKey: .other)
//                            amountPerCategory[.other]! += Float((transaction.amount! / 100))
//                        } else {
////                            let value = Float((transaction.amount! / 100))
////                            amountPerCategory.updateValue(value, forKey: .other)
//                            amountPerCategory[.other] = Float((transaction.amount! / 100))
//                            categoriesWithValue.append(.other)
//                        }
//                    }
                }
            }
//            self.mcc.subs
            ...
            //            if transaction.mcc == self.mmc.
        }
        
        for (a, b) in amountPerCategory {
            print("category - \(a), amount - \(b)")
        }
//        print("amountPerCategory - \(amountPerCategory)")
